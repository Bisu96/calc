...
public class Calculator2 extends JFrame implements ActionListener, KeyListener{
 ...
 public Calculator2(){
  ...
  //연산버튼추가!!!!!!!!!!!!!!!!!!!!!!!!
  buttons[23] = new JButton("ABS");	//new
  buttons[24] = new JButton("x²");	//new
  buttons[25] = new JButton("!");	//new
  buttons[26] = new JButton("log");	//new
  buttons[27] = new JButton("Σ");	//new
  buttons[28] = new JButton("sin");	//new
  buttons[29] = new JButton("cos");	//new
  buttons[30] = new JButton("tan");	//new
  ...
  for(int i=0 ; i<buttons.length ; i++){
   ...
   else if(i>=10 && i<21 || i==23 || i==24 || i==25 || i==26||i==27||i==28 ||i==29||i==30)
   {
 ...
 //기존 btnPanel.setLayout(new GridLayout(4,5,2,2));
  btnPanel.setLayout(new GridLayout(6,5,2,2));
  ...
  //new!!!!!!!!!!!!!
  btnPanel.add(buttons[23]); // ABS
  btnPanel.add(buttons[24]); // 제곱
  btnPanel.add(buttons[25]); // 팩토리얼
  btnPanel.add(buttons[26]); // 로그
  btnPanel.add(buttons[27]); // 시그마
  btnPanel.add(buttons[28]); // sin
  btnPanel.add(buttons[29]); // cos
  btnPanel.add(buttons[30]); // tan
      ...
      case 28: // sin
    	  if (displayMode != ERROR_MODE)
          {
           ...
           {       
            	...
            	result = Math.sin(getNumberInDisplay()* Math.PI/180);
      ...
      case 30: // tan
    	  if (displayMode != ERROR_MODE)
          {
             try
           {
            	...
            	result = Math.tan(getNumberInDisplay()* Math.PI/180);
 ...
 private double processLastOperator() throws Exception{
  ...
  //new!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if (lastOperator.equals("log")){
	   result = Math.log(numberInDisplay) / Math.log(lastNumber);
	  }
  
  if (lastOperator.equals("Σ")){
	  int sum = 0;
	  int x=(int) lastNumber;
	  int y=(int) numberInDisplay;
      for(int j=x; j<y+1; j++){
          sum += j;
       }
	   result = sum;
